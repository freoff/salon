...
export class ClientEventsRepository {
  ...
  createClientEvent(
    ...
    return this.rxdb.getDb$().pipe(
      ...
      switchMap((db) => from(this.insertClientEventToDb(db, clientEvent, client))),
    );
  }
  ...
  updateClientEvent(clientUpdatePayload) {
    let ce: Partial<ClientEvent> | any = { ...clientUpdatePayload.data };
    let eventId;
    if (ce.hasOwnProperty('_id')) {
      eventId = ce._id;
      delete ce._id;
    } else {
      eventId = ce.eventId;
      ce = { eventNotes: clientUpdatePayload.newText };
    }

    return this.db$.pipe(
      exhaustMap((db) => db.client_events.findOne(eventId).$.pipe(first())),
      map(async (clientEventDoc) => await clientEventDoc.atomicUpdate((oldData) => ({ ...oldData, ...ce }))),
    );
  }
  deleteClientEvent({ clientEventId }): Observable<boolean> {
    return this.db$.pipe(exhaustMap((db) => from(this.deleteEventFromDb(db, clientEventId))));
  }
  private deleteEventFromDb(db: SalonDatabase, id) {
    return db.client_events
      .findOne(id)
      .exec()
