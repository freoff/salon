...
export class ApplicationEffects {
  ...
  restoreBackup$ = this.actions$.pipe(
    ...
    switchMap((action) => from(this.rxdb.getDb().then((db) => db.importDump(JSON.parse(action.payload.json)))).pipe(
      map(success => new DisplayToast({toastOptions: {message: 'messages.restoreDBSuccess'}})),
      catchError(error => of(new DisplayToast({toastOptions: {message: 'messages.restoreDBFail', color: 'danger'}})))
    )),
  );
  saveJson(obj) {
